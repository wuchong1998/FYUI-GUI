...
namespace FYUI {
	...
	void CVirListBodyUI::InitElement(int nMaxItemCount /*= 50*/)
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (pControl != nullptr) 
			{
				...
				this->Add(pControl);
				static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")))
					->SetOwner(m_pOwner);
				static_cast<IListItemUI*>(
					GetItemAt(i)->GetInterface(_T("ListItem")))
					->SetIndex(i);
			}
		}
	}
	...
	void CVirListBodyUI::Refresh()
	{
		...
		if (UseDefaultLayout())
		{
			...
			for (int i = 0; i < (int)m_items.GetSize(); i++)
			{
				...
				{
					bIsFirst = false;
					FillElement((CControlUI*)m_items[i],i,false);
				} else 
				{
					FillElement((CControlUI*)m_items[i],i,false);
				}
	...
	void CVirListBodyUI::RemoveAll(bool bChildDelayed)
	{
		__super::RemoveAll();
	...
	void CVirListBodyUI::SetPos(RECT rc, bool bNeedInvalidate /*= true*/)
	{
		...
		ProcessScrollBar(rc, cxNeeded, (GetElementCount() + 1)*m_nOwnerElementHeight);
		//SetPosInternally(rc, cxNeeded);
	...
	void CVirListBodyUI::SetPosInternally(RECT rc, int cxRequired)
	{
		rc = m_rcItem;
		rc.bottom = GetElementCount()*m_nOwnerElementHeight + rc.top;
		ProcessScrollBar(m_rcItem, cxRequired, rc.bottom - rc.top);
	}
	...
	void CVirListBodyUI::ReArrangeChild(bool bForce)
	{
		...
		for (int i = 0; i < (int)m_items.GetSize(); i++)//重新改变元素在容器中的位置
		{
			...
			if (m_HorRect.left != m_HorRect.right) 
			{
				rcItem.left = m_HorRect.left;
				rcItem.right = m_HorRect.right;
			}
			...
			if (nElementIndex < nElementCount)//
			{
				...
				{
					bIsFirst = false;
					FillElement(lp, nElementIndex, true);
				} else 
				{
					FillElement(lp, nElementIndex, false);
				} 
	...
	void CVirListBodyUI::AddElement(int iIndex)
	{
		...
		if (nCount <= m_nOwnerItemCount)
		{
			...
			{
				bIsFirst = false;
				FillElement(pControl, iIndex, true);
			} else 
			{
				FillElement(pControl, iIndex, false);
			}
	...
	FYUI::CControlUI* CVirListBodyUI::CreateElement()
	{
		if (m_pDataProvider)
		{
	...
	void CVirListBodyUI::FillElement(CControlUI* pControl,int iIndex,bool bIsFirst)
	{
		if (m_pDataProvider) {
			if (bIsFirst) {
	...
	void CVirListBodyUI::InitSelectItem()
	{
		...
		for (size_t i = 0; i < m_vSelIndex.size(); ++i)
		{
	...
	bool CVirListBodyUI::SelectItem(int iIndex,bool bTakeFocus /*= false*/,bool bIsClick /*= false*/)
	{
	...
	void CVirListBodyUI::UnSelectAllItems()
	{
		...
		for (size_t i = 0; i < m_vSelIndex.size(); ++i)//将容器中选中的设置为不选中
		{
			int iIndex = m_vSelIndex[i] - nTopIndex;
	...
	bool CVirListBodyUI::GetSelectIndex(std::vector<int>& vec) {
		if (!m_vSelIndex.empty())
		{
			vec = m_vSelIndex;
			return true;
		}
