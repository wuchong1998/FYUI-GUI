...
namespace FYUI 
{
    ...
    namespace
    {
        // Native pixel scrolling can desync the window surface and the D2D back-buffer,
        // leaving ghost text during high-DPI wheel bursts. Prefer deterministic repaint.
        constexpr bool kEnableScrollRenderCache = false;
        constexpr int kWheelAnimationTargetTicksPerNotch = 16;
        constexpr int kWheelAnimationMaxTicks = 48;
        constexpr UINT kWheelAnimationTimerMs = 7;
        constexpr auto kScrollPresentMinInterval = std::chrono::microseconds(7000);

        UINT& WheelTimerResolutionRefCount()
        {
            static UINT refCount = 0;
            return refCount;
        }

        void BeginWheelTimerResolution()
        {
            UINT& refCount = WheelTimerResolutionRefCount();
            if (refCount++ == 0) {
                ::timeBeginPeriod(1);
            }
        }

        void EndWheelTimerResolution()
        {
            UINT& refCount = WheelTimerResolutionRefCount();
            if (refCount == 0) {
                return;
        ...
        std::chrono::steady_clock::time_point& LastScrollPresentTime()
        {
            static std::chrono::steady_clock::time_point lastPresentTime = {};
            return lastPresentTime;
        }

        bool& ScrollPresentReentryGuard()
        {
            static bool isPresenting = false;
            return isPresenting;
        }

        void PresentPendingScrollFrameIfDue(CPaintManagerUI* pManager)
        {
            if (pManager == nullptr || ScrollPresentReentryGuard()) {
                return;
            }

            HWND hWndPaint = pManager->GetPaintWindow();
            if (hWndPaint == nullptr || !::IsWindow(hWndPaint)) {
                return;
            }

            const auto now = std::chrono::steady_clock::now();
            auto& lastPresentTime = LastScrollPresentTime();
            if (lastPresentTime.time_since_epoch().count() != 0 &&
                now - lastPresentTime < kScrollPresentMinInterval) {
                return;
            }

            lastPresentTime = now;
            ScrollPresentReentryGuard() = true;
            ::UpdateWindow(hWndPaint);
            ScrollPresentReentryGuard() = false;
        }

        bool ContainsNativeScrollHost(CControlUI* pControl)
        {
            if (pControl == nullptr) {
                return false;
            }
            if (pControl->GetInterface(DUI_CTR_RICHEDIT) != nullptr) {
                return true;
            }
            if (!pControl->IsContainerControl()) {
                return false;
            }

            CContainerUI* pContainer = static_cast<CContainerUI*>(pControl);
            for (int index = 0; index < pContainer->GetCount(); ++index) {
                if (ContainsNativeScrollHost(pContainer->GetItemAt(index))) {
                    return true;
                }
            }
            return false;
        ...
        bool CanScrollBarInDirection(CScrollBarUI* pScrollBar, int direction)
        {
            if (pScrollBar == nullptr || !pScrollBar->IsEnabled()) {
                return false;
            }

            const int range = pScrollBar->GetScrollRange();
            if (range <= 0) {
                return false;
            }

            const int pos = pScrollBar->GetScrollPos();
            return direction < 0 ? pos > 0 : pos < range;
        }

        int ResolveWheelAnimationTicks(int remaining, int step)
        {
            ...
            const int wheelStep = step < 0 ? -step : step;
            const int targetPixelsPerTick = (std::max)(1, (wheelStep + kWheelAnimationTargetTicksPerNotch - 1) / kWheelAnimationTargetTicksPerNotch);
            int ticks = (distance + targetPixelsPerTick - 1) / targetPixelsPerTick;
            if (ticks > kWheelAnimationMaxTicks) {
                ticks = kWheelAnimationMaxTicks;
            }
            return ticks;
        }

        void CALLBACK WheelAnimationTimerCallback(UINT, UINT, DWORD_PTR user, DWORD_PTR, DWORD_PTR)
        {
            auto* pContainer = reinterpret_cast<CContainerUI*>(user);
            if (pContainer != nullptr) {
                pContainer->PostWheelAnimationMessageFromTimer();
            }
        }

        void OffsetControlOnlyForScroll(CControlUI* pControl, int dx, int dy)
        {
            if (pControl == nullptr || (dx == 0 && dy == 0)) {
                return;
            }

            RECT rc = pControl->GetPos();
            ::OffsetRect(&rc, dx, dy);
            pControl->SetOnlyPos(rc);
        }

        void OffsetControlTreeForScroll(CControlUI* pControl, int dx, int dy)
        {
            if (pControl == nullptr || (dx == 0 && dy == 0)) {
                return;
            }
            ...
            OffsetControlOnlyForScroll(pContainer->GetVerticalScrollBar(), dx, dy);
            OffsetControlOnlyForScroll(pContainer->GetHorizontalScrollBar(), dx, dy);

            const int nCount = pContainer->GetCount();
            for (int i = 0; i < nCount; ++i) {
                CControlUI* pChild = pContainer->GetItemAt(i);
                if (pChild != nullptr && pChild->IsVisible()) {
                    OffsetControlTreeForScroll(pChild, dx, dy);
                }
            }
        }
    }
        ...
        m_bInertiaActive = false;

        // 閺傛澘顤冮崝銊ф暰閸掓繂顫愰崠?

        m_nWheelAnimPerTick = 0;
        m_nWheelAnimRemainder = 0;
        m_nWheelAnimRemainingTicks = 0;
        m_bWheelAnimating = false;
        m_bWheelScrollVisible = false;
    }
    ...
    void CContainerUI::CopyData(CContainerUI* pControl)
    {
        ...
        m_nWheelAnimPerTick = pControl->m_nWheelAnimPerTick;

        m_nWheelAnimRemainder = pControl->m_nWheelAnimRemainder;
        m_nWheelAnimRemainingTicks = pControl->m_nWheelAnimRemainingTicks;
        m_bWheelAnimating = pControl->m_bWheelAnimating;
        m_bWheelScrollVisible = pControl->m_bWheelScrollVisible;
        m_nWheelAnimTargetTotalY = pControl->m_nWheelAnimTargetTotalY;
        m_nWheelAnimMovedY = pControl->m_nWheelAnimMovedY;
        m_bInertiaActive = pControl->m_bInertiaActive;
    ...
    void CContainerUI::DoEvent(TEventUI& event)
    {
        ...
        if (event.Type == UIEVENT_TIMER && event.wParam == kInertiaTimerId) {
            InertiaTick();
            return;
        }
        ...
        if (event.Type == UIEVENT_SCROLLWHEEL) {
            if (m_pVerticalScrollBar && m_pVerticalScrollBar->IsEnabled()) {
                if ((event.wKeyState & MK_CONTROL) == 0) {
                    ...
                    const int zDelta = static_cast<short>(HIWORD(event.wParam));
                    int wheelSteps = ::abs(zDelta) / WHEEL_DELTA;
                    if (wheelSteps < 1) {
                        wheelSteps = 1;
                    }
                    ...
                    if (code == SB_LINEUP)
                    {
                        if (!CanScrollBarInDirection(m_pVerticalScrollBar, -1)) {
                            StopWheelAnimation();
                            CControlUI::DoEvent(event);
                            return;
                        }
                        ...
                        {
                            ApplyWheelAcceleration(-wheelSteps, m_pVerticalScrollBar->IsVisible());
                        }
                        else
                        {
                            for (int index = 0; index < wheelSteps; ++index) {
                                LineUp();
                            }
                        }
    ...
    void CContainerUI::SetScrollPos(SIZE szPos, bool bMsg, bool bScroolVisible) {
        ...
        for (int it2 = 0; it2 < m_items.GetSize(); it2++)
        {
            ...
            ::OffsetRect(&rcPos, dx, dy);
            if (m_bQuickScrolling && (rcPos.bottom < m_rcItem.top || rcPos.top > m_rcItem.bottom))
            {
                pControl->SetOnlyPos(rcPos);
                continue;
            }

            OffsetControlTreeForScroll(pControl, dx, dy);
        }
        ...
        if (m_pManager != nullptr) {
            ...
            if (m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible()) {
                rcScroll.right -= m_pVerticalScrollBar->GetFixedWidth();
            }
            if (m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible()) {
                rcScroll.bottom -= m_pHorizontalScrollBar->GetFixedHeight();
            }
        }

        bool usedPixelScroll = false;
        if (m_pManager != nullptr && kEnableScrollRenderCache && !ContainsNativeScrollHost(this)) {
            usedPixelScroll = m_pManager->ScrollRenderCacheRect(rcScroll, dx, dy);
        }

        if (!usedPixelScroll) {
            if (m_pManager != nullptr) {
                m_pManager->Invalidate(rcScroll);
                if (m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible()) {
                    RECT rcVerticalScrollBar = m_pVerticalScrollBar->GetPos();
                    m_pManager->Invalidate(rcVerticalScrollBar);
                }
                if (m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible()) {
                    RECT rcHorizontalScrollBar = m_pHorizontalScrollBar->GetPos();
                    m_pManager->Invalidate(rcHorizontalScrollBar);
                }
            }
            else {
                Invalidate();
        ...
        if (m_pVerticalScrollBar) {
            ...
            if (m_pManager != NULL && bMsg) {
                int nPage = (m_pVerticalScrollBar->GetScrollPos() +
        ...
        PresentPendingScrollFrameIfDue(m_pManager);
    }
    ...
    void CContainerUI::LineUp(bool bScroolVisible) {
        ...
        sz.cy -= cyLine;
        SetScrollPos(sz, bScroolVisible);
    }
    ...
    void CContainerUI::LineDown(bool bScroolVisible) {
        ...
        SIZE sz = GetScrollPos();
        sz.cy += cyLine;
        SetScrollPos(sz, true, bScroolVisible);
    }
    ...
    void CContainerUI::ApplyWheelAcceleration(int direction, bool bScrollVisible)
    {
        int step = GetScrollStepSize();
        if (step == 0)
        {
            if (m_pManager)
                step = m_pManager->ScaleValue(120);
        }
        const bool wasAnimating = m_bWheelAnimating;
        const int delta = direction * step;
        if (m_bWheelAnimating) {
            if ((m_nWheelAnimTargetTotalY - m_nWheelAnimMovedY) * delta < 0) {
                m_nWheelAnimTargetTotalY = delta;
                m_nWheelAnimMovedY = 0;
            }
            else {
                m_nWheelAnimTargetTotalY += delta;
            }
        ...
        const int remaining = m_nWheelAnimTargetTotalY - m_nWheelAnimMovedY;
        if (remaining == 0) {
            StopWheelAnimation();
            return;
        }

        m_nWheelAnimRemainingTicks = ResolveWheelAnimationTicks(remaining, step);
        m_nWheelAnimPerTick = remaining / m_nWheelAnimRemainingTicks;
        m_nWheelAnimRemainder = remaining - m_nWheelAnimPerTick * m_nWheelAnimRemainingTicks;
        m_bWheelAnimating = true;
        m_bWheelScrollVisible = bScrollVisible;

        if (!wasAnimating) {
            BeginWheelTimerResolution();
            InertiaTick();
            if (m_bWheelAnimating && m_pManager) {
                m_uWheelAnimationTimerId = ::timeSetEvent(
                    kWheelAnimationTimerMs,
                    1,
                    WheelAnimationTimerCallback,
                    reinterpret_cast<DWORD_PTR>(this),
                    TIME_PERIODIC | TIME_CALLBACK_FUNCTION);
                if (m_uWheelAnimationTimerId == 0) {
                    m_pManager->SetTimer(this, kInertiaTimerId, kWheelAnimationTimerMs);
                }
            }
        }
        else if (m_bWheelAnimating && m_pManager && m_uWheelAnimationTimerId == 0) {
            m_uWheelAnimationTimerId = ::timeSetEvent(
                kWheelAnimationTimerMs,
                1,
                WheelAnimationTimerCallback,
    ...
    void CContainerUI::PostWheelAnimationMessageFromTimer()
    {
        if (!m_bWheelAnimating || m_pManager == nullptr) {
            return;
        }

        HWND hWndPaint = m_pManager->GetPaintWindow();
        if (hWndPaint == nullptr || !::IsWindow(hWndPaint)) {
            return;
        }

        if (::InterlockedCompareExchange(&m_lWheelAnimationMessagePending, 1, 0) == 0) {
            if (!::PostMessage(hWndPaint, UIMSG_SCROLL_ANIMATE, reinterpret_cast<WPARAM>(this), 0)) {
                ::InterlockedExchange(&m_lWheelAnimationMessagePending, 0);
            }
        }
    }

    bool CContainerUI::HandleWheelAnimationFrameMessage()
    {
        ::InterlockedExchange(&m_lWheelAnimationMessagePending, 0);
        if (!m_bWheelAnimating) {
            return false;
    ...
    void CContainerUI::StopWheelAnimation()
    {
        const bool wasAnimating = m_bWheelAnimating;
        if (!wasAnimating && m_uWheelAnimationTimerId == 0) {
            if (m_pManager) m_pManager->KillTimer(this, kInertiaTimerId);
            ::InterlockedExchange(&m_lWheelAnimationMessagePending, 0);
            return;
        }

        if (m_uWheelAnimationTimerId != 0) {
            ::timeKillEvent(m_uWheelAnimationTimerId);
            m_uWheelAnimationTimerId = 0;
        }
        if (m_pManager) m_pManager->KillTimer(this, kInertiaTimerId);
        ::InterlockedExchange(&m_lWheelAnimationMessagePending, 0);
        m_bWheelAnimating = false;
        if (wasAnimating) {
            EndWheelTimerResolution();
        }

        m_nWheelAnimPerTick = 0;
        m_nWheelAnimRemainder = 0;
        m_nWheelAnimRemainingTicks = 0;
        m_nWheelAnimTargetTotalY = 0;
        m_nWheelAnimMovedY = 0;
    ...
    void CContainerUI::InertiaTick()
    {
        if (!m_bWheelAnimating) return;

        int remaining = m_nWheelAnimTargetTotalY - m_nWheelAnimMovedY;
        if (remaining == 0 || m_nWheelAnimRemainingTicks <= 0) {
            StopWheelAnimation();
            return;
        }

        int deltaThisTick = m_nWheelAnimPerTick;
        if (m_nWheelAnimRemainder != 0) {
            const int adjust = (m_nWheelAnimRemainder > 0) ? 1 : -1;
            deltaThisTick += adjust;
            m_nWheelAnimRemainder -= adjust;
        }

        if ((deltaThisTick > 0 && deltaThisTick > remaining) ||
            (deltaThisTick < 0 && deltaThisTick < remaining)) {
            deltaThisTick = remaining;
        }

        const SIZE szBefore = GetScrollPos();
        SIZE sz = szBefore;
        sz.cy += deltaThisTick;
        SetScrollPos(sz, true, m_bWheelScrollVisible);

        const SIZE szAfter = GetScrollPos();
        const int moved = szAfter.cy - szBefore.cy;
        m_nWheelAnimMovedY += moved;
        --m_nWheelAnimRemainingTicks;
        if (moved == 0 && deltaThisTick != 0) {
            StopWheelAnimation();
            return;
        }

        remaining = m_nWheelAnimTargetTotalY - m_nWheelAnimMovedY;
        if (remaining == 0 || m_nWheelAnimRemainingTicks <= 0) {
            StopWheelAnimation();
        }
    }
