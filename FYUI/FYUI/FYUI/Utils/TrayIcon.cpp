...
namespace FYUI
{
	...
	void CTrayIcon::CreateTrayIcon(HWND _RecvHwnd, UINT _IconIDResource, std::wstring_view _ToolTipText, UINT _Message)
	{
	   ...
	   m_hIcon = LoadIcon(CPaintManagerUI::GetInstance(), MAKEINTRESOURCE(_IconIDResource));
		...
		{
			const std::wstring toolTipText(_ToolTipText);
			wcscpy_s(m_trayData.szTip, _countof(m_trayData.szTip), toolTipText.c_str());
		}
	...
	bool CTrayIcon::SetTooltipText(std::wstring_view _ToolTipText)
	{
		if(!_ToolTipText.empty())
		{
			const std::wstring toolTipText(_ToolTipText);
	...
	std::wstring CTrayIcon::GetTooltipText() const
	{
	...
	bool CTrayIcon::SetIcon(std::wstring_view _IconFile)
	{
		const std::wstring iconFile(_IconFile);
		HICON hIcon = LoadIcon(CPaintManagerUI::GetInstance(), iconFile.c_str());
