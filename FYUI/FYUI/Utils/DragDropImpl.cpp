...
namespace FYUI {
	...
	{ 
		...
		if(fRelease) {
			...
			CopyMedium(&stgMed, pmedium, pformatetc);
		}

		const size_t originalFormatCount = m_ArrFormatEtc.size();
		try {
			m_ArrFormatEtc.push_back(fetc);
			m_StgMedium.push_back(stgMed);
		}
		catch(...) {
			if(m_ArrFormatEtc.size() > originalFormatCount) {
				m_ArrFormatEtc.pop_back();
			}
			if(!fRelease) {
	...
	void CIDataObject::CopyMedium(STGMEDIUM* pMedDest, const STGMEDIUM* pMedSrc, const FORMATETC* pFmtSrc)
	{
		if(pMedDest == NULL || pMedSrc == NULL || pFmtSrc == NULL) {
			return;
		}

		ZeroMemory(pMedDest, sizeof(STGMEDIUM));
		...
		case TYMED_ISTREAM:
			...
			if(pMedDest->pstm != NULL) {
				pMedDest->pstm->AddRef();
			}
			break;
		case TYMED_ISTORAGE:
			pMedDest->pstg = pMedSrc->pstg;
			if(pMedDest->pstg != NULL) {
