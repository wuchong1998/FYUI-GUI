...
// 增加动态库导出宏，假设你的 FYUI 预处理器中定义了 FYUI_EXPORTS 
// (如果没有定义，请在项目属性 -> C/C++ -> 预处理器 -> 预处理器定义 中添加 FYUI_EXPORTS)
#ifndef UNZIP_API
#ifdef FYUI_STATIC
#define UNZIP_API
#elif defined(FYUI_EXPORTS)
...
{ int index;                 // index of this file within the zip
  wchar_t name[MAX_PATH];      // filename within the zip
...
UNZIP_API HZIP OpenZip(const wchar_t *fn, const char *password=NULL);
UNZIP_API HZIP OpenZip(void *z, unsigned int len, const char *password=NULL);
UNZIP_API HZIP OpenZipHandle(HANDLE h, const char *password);
...
UNZIP_API ZRESULT GetZipItem(HZIP hz, int index, ZIPENTRY *ze);
...
UNZIP_API ZRESULT FindZipItem(HZIP hz, const wchar_t *name, bool ic, int *index, ZIPENTRY *ze);
// FindZipItem - finds an item by name. ic means 'insensitive to case'.
// It returns the index of the item, and returns information about it.
// If nothing was found, then index is set to -1 and the function returns
// an error code.

UNZIP_API ZRESULT UnzipItem(HZIP hz, int index, const wchar_t *fn);
UNZIP_API ZRESULT UnzipItem(HZIP hz, int index, void *z, unsigned int len);
UNZIP_API ZRESULT UnzipItemHandle(HZIP hz, int index, HANDLE h);
...
UNZIP_API ZRESULT SetUnzipBaseDir(HZIP hz, const wchar_t *dir);
// if unzipping to a filename, and it's a relative filename, then it will be relative to here.
// (defaults to current-directory).


UNZIP_API ZRESULT CloseZip(HZIP hz);
// CloseZip - the zip handle must be closed with this function.

UNZIP_API unsigned int FormatZipMessage(ZRESULT code, wchar_t *buf, unsigned int len);
...
UNZIP_API ZRESULT CloseZipU(HZIP hz);
UNZIP_API unsigned int FormatZipMessageU(ZRESULT code, wchar_t *buf, unsigned int len);
UNZIP_API bool IsZipHandleU(HZIP hz);
#ifdef _zip_H
#undef CloseZip
