...
if (!(Test-Path -LiteralPath $ProjectFile)) {
    ...
    $requiredProjectIncludes = @(
        ...
        'Core\Render\UIRenderClipStackInternal.h',
    ...
    foreach ($item in $projectItems) {
        if ($item.Include -notmatch '(?i)^Core\\UIRenderClipStackInternal\.h$') {
            continue
        }

        $text = Get-Content -LiteralPath $item.Path -Raw
        $clipStackHeaderBans = @(
            @{ Pattern = '(?i)\bHDC\b'; Message = 'Clip-stack internal header must stay D2D-context based, not HDC-based.' }
        )
        foreach ($ban in $clipStackHeaderBans) {
            $match = [regex]::Match($text, $ban.Pattern)
            if ($match.Success) {
                Add-Failure "$($ban.Message) ($($item.Include):$((Get-LineNumber $text $match.Index)))"
            }
